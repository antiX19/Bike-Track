...
public interface ApiService {
   ...
   Call<VeloData> postVeloData(@Body VeloData veloData);
    ...
    @POST("/reset-password/question")
        // Pour envoyer des données
    Call<Resepsw> postNewpsw(@Body Resepsw resepsw);
    ...
    @POST("dh-exchange")
    Call<PublicKey_new> postpublickey(@Body PublicKey_new publicKey);

    @GET("dh-exchange/{uuid}")
    Call<PublicKey_new> getPublicKey(@Path("uuid") String UUID_Velo);

    @GET("secret-questions")
    Call<List<Secretquestion>> getscretquestion();
